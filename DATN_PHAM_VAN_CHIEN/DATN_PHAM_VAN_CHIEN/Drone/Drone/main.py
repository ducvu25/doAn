...
def assign_targets_to_drones(drones_positions, target_positions):
    num_drones = len(drones_positions)
    cost_matrix = np.zeros((num_drones, num_drones))

    for i in range(num_drones):
        for j in range(num_drones):
            cost_matrix[i][j] = np.linalg.norm(np.array(drones_positions[i]) - np.array(target_positions[j]))

    row_ind, col_ind = linear_sum_assignment(cost_matrix)
    assigned_targets = [target_positions[j] for j in col_ind]
    return assigned_targets

K_ATTRACTIVE = 1.0  # Hằng số lực hấp dẫn
K_REPULSIVE = 5.0  # Hằng số lực đẩy tránh
Q_STAR = 4.0  # Khoảng cách giới hạn để tính lực đẩy tránh
R = 1.0  # Khoảng cách để tính lực hấp dẫn cải tiến
rho_ng_i = 0.3  # Khoảng cách thêm vào (có thể là giá trị từ dữ liệu)
theta = 0.05  # Góc lệch giữa UAV và mục tiêu (đơn vị radian)

def calculate_attractive_force(current_position, goal_position):
    """
    Tính lực hấp dẫn giữa UAV và mục tiêu.
    Lực hấp dẫn theo phương vector giữa vị trí hiện tại và vị trí mục tiêu.
    """
    # Tính toán lực hấp dẫn theo công thức
...
def calculate_repulsive_force(current_position, obstacles):
    """
    Tính lực đẩy tránh từ các chướng ngại vật theo công thức APF cải tiến.
    """
    fx, fy, fz = 0.0, 0.0, 0.0
    for obs in obstacles:
        # Tính khoảng cách giữa UAV và chướng ngại vật
        dist = math.sqrt((current_position[0] - obs[0])**2 + 
                         (current_position[1] - obs[1])**2 + 
                         (current_position[2] - obs[2])**2)
        
        # Nếu UAV ở gần chướng ngại vật (khoảng cách nhỏ hơn Q_STAR), tính lực đẩy tránh
        if dist < Q_STAR:
            # Áp dụng công thức cải tiến để tính lực đẩy tránh
            repulsive_strength = K_REPULSIVE * ((1.0 / dist - 1.0 / R) ** 2) * rho_ng_i * (math.cos(theta)/dist**2)
...
def calculate_total_force(current_position, goal_position, obstacles):
    """
    Tính tổng lực tác động lên UAV (gồm lực hấp dẫn và lực đẩy tránh).
    """
    # Tính lực hấp dẫn
    ...
    # Tổng hợp lực (lực hấp dẫn + lực đẩy tránh)
...
async def get_global_position_telemetry(drone_id, drone):
    try:
        ...
        print(f"Error in get_global_position_telemetry for drone {drone_id}: {e}")
...
async def initial_setup_and_connection(drone_id, udp_port):
    ...
    try:
        ...
        print(f"Error during initial setup for drone {drone_id}: {e}")

    await asyncio.sleep(5)  # Adding delay to ensure connection is stable
...
async def pre_flight_checks(drone_id, drone):
    try:
        ...
        print(f"Error in pre_flight_checks for drone {drone_id}: {e}")
...
async def arming_and_starting_offboard_mode(drone_id, drone):
    try:
        ...
        print(f"Error in arming_and_starting_offboard_mode for drone {drone_id}: {e}")
...
async def perform_landing(drone_id, drone):
    try:
        ...
        print(f"Error in perform_landing for drone {drone_id}: {e}")
...
async def disarm_drone(drone_id, drone):
    try:
        ...
        print(f"Error in disarm_drone for drone {drone_id}: {e}")
...
async def create_drone_configurations(num_drones, time_offset):
    ...
    traejctory_offset = [(0, 0, 0) for _ in range(num_drones)]
    if not SIM_MODE:
        udp_ports = [14540]  # default API connection on same hardware
    else:
...
async def run_all_drones(num_drones, home_positions, traejctory_offset, udp_ports, time_offset, altitude_offsets):
    tasks = []
    drones_positions = [(drone.x, drone.y, DEFAULT_Z) for drone in dronesConfig]
    assigned_targets = assign_targets_to_drones(drones_positions, home_positions)

    for i in range(num_drones):
        drone_id = i if SIM_MODE else HW_ID
        tasks.append(asyncio.create_task(run_drone(drone_id, assigned_targets[i], traejctory_offset[i], udp_ports[i], i * time_offset, altitude_offsets[i])))
...
async def run_drone(drone_id, home_position_NED, trajectory_offset, udp_port, time_offset, altitude_offset):
    ...
    if SEPERATE_CSV:
        filename = f"shapes/swarm/processed/Drone {HW_ID if not SIM_MODE else drone_id + 1}.csv"
    else:
