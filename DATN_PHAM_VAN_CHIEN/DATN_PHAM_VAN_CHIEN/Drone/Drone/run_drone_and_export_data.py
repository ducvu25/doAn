...
async def perform_trajectory(drone_id, drone, waypoints, home_position, home_position_NED, global_position_telemetry, mode_descriptions):
    ...
    while t <= total_duration:
        ...
        trajectory_data.append((
            t, *position, *velocity, *acceleration, yaw, mode_code
        ))
        await asyncio.sleep(STEP_TIME)
        t += STEP_TIME
...
async def run_drone(drone_id, home_position_NED, trajectory_offset, udp_port, time_offset, altitude_offset):
    ...
    trajectory_data = await perform_trajectory(drone_id, drone, waypoints, home_position, home_position_NED, global_position_telemetry, mode_descriptions)
    # trajectory_data = await perform_trajectory(drone, waypoints, mode_descriptions)
    await save_trajectory_to_csv("drone_trajectory Drone " + str(drone_id + 1) + ".csv", trajectory_data)
    # Perform landing
